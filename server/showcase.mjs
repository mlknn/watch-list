...
export async function showcase(){if(cached&&Date.now()-cached.at<15000)return cached.data;if(pending)return pending;
 pending=(async()=>{const entries=new Array(examples.length);let next=0;await Promise.all(Array.from({length:4},async()=>{while(next<examples.length){const i=next++,example=examples[i];const [bars,current]=await Promise.all([history(example.symbol),getQuote(example.symbol)]);entries[i]={example,bars,current};}}));const data=buildShowcase(entries);cached={at:Date.now(),data};return data;})();try{return await pending;}finally{pending=null;}
}
