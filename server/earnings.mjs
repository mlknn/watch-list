import {requireStockAccess} from './stock-access.mjs';
...
export async function earningsForRequest(request,value,{authenticate=requireUser,access=requireStockAccess,load=fetchEarnings}={}){
 const {db,user}=await authenticate(request);const profile=dbResult(await db.from('wl_profiles').select('*').eq('id',user.id).single());if(!profile||planFor(profile).id!=='pro')throw new AppError('Earnings reports are included with Pro.',403);const {symbol}=await access(request,value,async()=>({db,user}));await rate(db,'earnings:'+user.id,30,60);return load(symbol);
}
