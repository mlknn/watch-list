...
export function persistLocalDatabase(){snapshotQueue=snapshotQueue.catch(()=>{}).then(async()=>{const pg=await localPg();await pg.exec('checkpoint');const tables=['auth.users','public.local_users','public.local_sessions','public.local_tokens','public.wl_profiles','public.wl_watchlists','public.wl_stocks','public.wl_quotes','public.wl_stripe_events','public.wl_rate_limits','public.wl_dashboard_favorites','public.wl_analytics_uniques','public.wl_analytics_totals'];const snapshot={version:1,savedAt:new Date().toISOString(),tables:{}};await pg.transaction(async tx=>{for(const table of tables)snapshot.tables[table]=(await tx.query('select * from '+table)).rows;});const target=join(root(),'accounts-recovery.json');await writeFile(target+'.tmp',JSON.stringify(snapshot),{mode:0o600});await rename(target+'.tmp',target);});return snapshotQueue;}
