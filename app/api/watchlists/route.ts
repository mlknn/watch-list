...
export async function GET(request:Request){try{const {db,user}=await requireUser(request);await rate(db,'read:'+user.id,90);const refresh=new URL(request.url).searchParams.get('refresh')==='1';return json(await accountState(db,user,refresh));}catch(e){return failure(e);}}
