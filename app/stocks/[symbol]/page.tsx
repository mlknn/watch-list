...
export default function StockDetails({params}:{params:Promise<{symbol:string}>}){const t=useT();
  ...
  const [quarterEnds,setQuarterEnds]=useState<string[]>([]);
  useEffect(()=>{if(crypto){setQuarterEnds([]);return;}let alive=true;void stockDataFetch('/api/stocks/'+encodeURIComponent(symbol)+'/earnings').then(async r=>{const value=await r.json() as {quarters?:{periodEnd:string}[]};if(alive)setQuarterEnds((value.quarters||[]).map(q=>q.periodEnd).filter(Boolean));}).catch(()=>{if(alive)setQuarterEnds([]);});return()=>{alive=false;};},[symbol,crypto]);
  ...
  const eventDates=chart?.earningsDates?.length?chart.earningsDates:quarterEnds;
  ...
  if(crypto){
    return <>
      ...
      <main className="stock-detail-page is-crypto-chart">
        <div className="stock-back-row"><a className="stock-back" href="/dashboard?market=crypto">{t("← Markets")}</a></div>
        <div className="stock-summary-card">
          <h1>{coinTitle} <span>({coinCode})</span></h1>
          {data&&<div className="detail-prices"><div className="detail-price"><strong>{price(data.quote.price,data.currency)}</strong>{change!==null&&change!==undefined&&<span className={direction}><ChangeIcon size={21}/>{change>=0?'+':''}{change.toFixed(2)}%</span>}</div></div>}
        </div>
