...
export default function Watchlists(){const t=useT();
  ...
  const lock=useRef(false),refreshing=useRef(false),generation=useRef(0),stateRef=useRef<AccountState|null>(null),inputRef=useRef<HTMLInputElement>(null),quantityRef=useRef<HTMLInputElement>(null),focusQuantity=useRef(false);
  ...
  useEffect(()=>{if(!focusQuantity.current||!ticker.trim())return;focusQuantity.current=false;quantityRef.current?.focus();quantityRef.current?.select();},[ticker]);
  ...
  <div className="watchlist-compose-row">{canEdit&&<form className={`add-bar ${active.mode==='advanced'?'advanced-add-bar':''}`} onSubmit={addStock}><StockSearch value={ticker} onChange={setTicker} onPick={()=>{focusQuantity.current=true;}} inputRef={inputRef} currency={active.stocks[0]?.currency}/><Button type="submit" className="primary-button" disabled={!!busy||!ticker.trim()||addLimited}>{busy==='addStock'?<LoaderCircle className="spin"/>:<Plus/>}{busy==='addStock'?t("Adding…"):t("Add stock")}</Button><Button type="button" variant="outline" className="refresh-button" disabled={!!busy} onClick={()=>void act({action:'refresh'}).catch(()=>{})}><RefreshCw className={busy==='refresh'?'spin':''}/><T text="Refresh"/></Button>{active.mode==='advanced'&&ticker.trim()&&<div className="add-quantity-step"><p className="add-quantity-label"><T text="Add quantity"/></p><PositionFields value={position} onChange={setPosition} currency={active.stocks[0]?.currency||'USD'} quantityRef={quantityRef}/></div>}</form>}<WatchlistSummary list={active}/></div>
