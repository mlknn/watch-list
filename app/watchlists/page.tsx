...
export default function Watchlists(){const t=useT();
  ...
  async function addStock(e:React.FormEvent){e.preventDefault();if(!active||!ticker.trim())return;try{const symbol=await resolveStockInput(ticker,active.stocks[0]?.currency);await act({action:'addStock',listId:active.id,ticker:symbol,...(active.mode==='advanced'?{quantity:Number(position.quantity),costPerShare:position.cost?Number(position.cost):undefined,acquiredAt:new Date(position.date+'T00:00:00').toISOString(),notes:position.notes}:{})});setNotice(`${ticker} added.`);setTicker('');setPosition(emptyPosition());inputRef.current?.focus();}catch(e){setError((e as Error).message);}}
  ...
  async function openShare(){if(state?.guest){setSaveOpen(true);return;}if(!active||lock.current)return;setError('');setNotice('');setDialog('share');if(!active.shareToken&&admin){try{await act({action:'shareList',listId:active.id});}catch{/* Show the retry action in the open dialog. */}}}
  ...
  return <><header className="topbar"><Brand/><div className="account-nav"><a href="/dashboard" onClick={warnLeave}>{t("Dashboard")}</a><a className="plan-badge" href="/pricing">{state?.plan.id==='pro'?'PRO':'BASIC'}</a>{state?.guest?<><a href="/login"><T text="Log in"/></a><button type="button" className="solid-link" onClick={()=>setSaveOpen(true)}><T text="Save portfolio"/></button></>:<><a href="/account"><T text="Account"/></a>{state?.user.analytics&&<a href="/insights">Analytics</a>}<span className="save-state">{busy?<LoaderCircle size={15} className="spin"/>:<Check size={15}/>} {busy?t("Updating…"):state?t("Saved to your account"):t("Connecting…")}</span></>}</div></header>
  <main className="workspace">{state&&!state.guest&&<div className="page-heading dashboard-heading"><div className="dashboard-heading-copy"><p className="eyebrow">{t("YOUR INVESTING SPACE")}</p><h1><T text="My watchlists"/></h1><p className="intro"><T text="Keep your ideas close. Follow how they grow."/></p><div className="workspace-summary"><span className="workspace-plan">{state.plan.id==='pro'?'Pro':t("Basic")}</span><span><strong>{state.watchlists.length}</strong> / {state.plan.maxLists} {t("Watchlists")}</span><span>{t("Stocks")} <strong>{state.plan.maxStocks}</strong></span></div></div><div className="heading-actions"><Button variant="outline" className="outline-button" disabled={!active||!!busy} onClick={()=>void openShare()}><Share2/><T text="Invite a friend"/></Button><Button className="primary-button" disabled={!state||!!busy} onClick={()=>{setName('');setError('');setDialog('create');}}><Plus/><T text="New watchlist"/></Button></div><WatchlistsOverview lists={state.watchlists} activeId={active?.id||''} onSelect={id=>{setActiveId(id);setTicker('');setError('');}}/></div>}
  ...
  {state&&active?<Tabs value={active.id} onValueChange={v=>{setActiveId(String(v));setTicker('');setError('');}}><div className="tab-strip"><TabsList variant="line" aria-label={t("Watchlists")}>{state.watchlists.map(l=><TabsTrigger key={l.id} value={l.id}>{l.name}<span className="tab-count">{l.stocks.length}</span></TabsTrigger>)}</TabsList></div><TabsContent key={active.id} value={active.id}><section className="watch-card"><div className="card-heading"><div><div className="list-title"><h2>{active.name}</h2><span className="owner-label">{active.mode==='advanced'?t("Advanced")+' · ':''}{admin?t("Owner"):t("Viewer")}</span>{admin&&<><button className="icon-button" aria-label="Rename watchlist" disabled={!!busy} onClick={()=>{setName(active.name);setError('');setDialog('rename');}}><Pencil size={14}/></button>{state.watchlists.length>1&&<button className="icon-button" aria-label="Delete watchlist" disabled={!!busy} onClick={()=>{setError('');setDeletion({listId:active.id,label:active.name});}}><Trash2 size={14}/></button>}</>}</div><p>{active.mode==='advanced'?t("Track your holdings, cost basis, and the bigger picture."):t("Track performance from the day you add a stock.")}</p></div><div className="card-tools">{state.guest&&<span className="save-state">{busy?<LoaderCircle size={15} className="spin"/>:<Check size={15}/>} {busy?t("Updating…"):t("Saved on this device")}</span>}<span className="refresh-state"><span className={`status-dot ${stale?'warning-dot':''}`}/>{busy==='refresh'?t("Refreshing…"):t("Refreshes every 60s")}</span><Button variant="outline" disabled={!!busy} onClick={()=>void openShare()}><Link2/><T text="Share"/></Button></div></div>
  <div className="watchlist-compose-row">{admin&&<form className={`add-bar ${active.mode==='advanced'?'advanced-add-bar':''}`} onSubmit={addStock}><StockSearch value={ticker} onChange={setTicker} inputRef={inputRef} currency={active.stocks[0]?.currency}/><Button type="submit" className="primary-button" disabled={!!busy||!ticker.trim()||addLimited}>{busy==='addStock'?<LoaderCircle className="spin"/>:<Plus/>}{busy==='addStock'?t("Adding…"):t("Add stock")}</Button><Button type="button" variant="outline" className="refresh-button" disabled={!!busy} onClick={()=>void act({action:'refresh'}).catch(()=>{})}><RefreshCw className={busy==='refresh'?'spin':''}/><T text="Refresh"/></Button>{active.mode==='advanced'&&<PositionFields value={position} onChange={setPosition} currency={active.stocks[0]?.currency||'USD'}/>}</form>}<WatchlistSummary list={active}/></div>
  ...
  <Dialog open={dialog==='create'||dialog==='rename'} onOpenChange={v=>{if(!v&&!busy)setDialog(null);}}><DialogContent className="list-dialog"><DialogTitle>{dialog==='create'?t("New watchlist"):t("Rename watchlist")}</DialogTitle><DialogDescription>{state&&dialog==='create'&&state.watchlists.length>=state.plan.maxLists?(state.plan.id==='pro'?`You’ve reached your limit of ${state.plan.maxLists} watchlists. Delete a watchlist to create another.`:'You’ve used all the watchlists in your plan. Upgrade to Pro for up to 10.'):dialog==='create'?'A name is optional. Leave it blank to use My watchlist.':'Keep your ideas together with a name that makes sense to you.'}</DialogDescription>{state&&dialog==='create'&&state.watchlists.length>=state.plan.maxLists?(state.plan.id==='pro'?<Button className="primary-button dialog-submit" onClick={()=>setDialog(null)}><T text="Back to watchlists"/></Button>:<a className="solid-link" href="/pricing">View plans <ArrowUpRight size={16}/></a>):<form onSubmit={saveList}><label className="form-label" htmlFor="list-name"><T text="Watchlist name"/></label><p className="field-note">{dialog==='create'?'Optional. Every watchlist includes quantities, costs and portfolio performance.':'Every watchlist includes quantities, costs and portfolio performance.'}</p><Input id="list-name" autoFocus maxLength={60} required={dialog==='rename'} placeholder="2027 AI Picks" value={name} onChange={e=>setName(e.target.value)}/>{error&&<p role="alert" className="form-error">{error}</p>}<Button type="submit" className="primary-button dialog-submit" disabled={!!busy||(dialog==='rename'&&!name.trim())}><T text="Save watchlist"/></Button></form>}</DialogContent></Dialog>
