...
test('guest lists persist across reload and survive a failed save attempt',()=>{
  ...
  state=applyGuestAction(state,{action:'createList',name:'Morning ideas'});
  ...
  assert.equal(restored.watchlists[0].name,'Morning ideas');
...
test('guest add requires a quote and keeps the draft when the action fails',()=>{
  const storage=memory();
  let state=applyGuestAction(emptyGuestState(),{action:'createList',name:'Draft'});
  writeGuestState(state,storage);
