...
test('changing the URL cannot access another member’s stock or an untracked stock',async()=>{for(const symbol of ['CRDO','IBM'])await assert.rejects(requireStockAccess(request,symbol,authenticate),e=>e.status===403);});
...
test('dashboard market stocks are public without authentication',async()=>{assert.equal((await requireStockAccess(request,'jpm',async()=>assert.fail('Public market stock requested login'))).symbol,'JPM');});
