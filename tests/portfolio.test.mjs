...
test('portfolio totals use quantities and cost basis; missing positions are explicit',()=>{assert.deepEqual(portfolioTotals([stock(),stock({quantity:20,costPerShare:20,currentPrice:15}),stock({quantity:null})]),{cost:1400,value:1500,gain:100,gainPercent:100/1400*100,positions:2,incomplete:1,currency:'USD'});});
test('portfolio cannot silently aggregate different currencies',()=>{assert.throws(()=>portfolioTotals([stock(),stock({currency:'EUR'})]),/currency/);assert.equal(portfolioTotals([stock({currency:'TRY'}),stock({currency:'TRY',quantity:20,costPerShare:20,currentPrice:15})]).currency,'TRY');});
