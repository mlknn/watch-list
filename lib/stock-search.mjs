import {tickerFitsCurrency} from './portfolio-currency.mjs';
const STOCKS = [
...
export async function resolveStockInput(query,currency){
 const text=String(query||'').trim();const suggestions=await searchTickerSuggestions(text,12,undefined,currency);
 ...
 if(bases.length===1)return bases[0].symbol;
