...
export function emptyGuestState(){
  return {
    ...
    user:{id:'guest',name:'Guest',email:'',isAdmin:true,local:true,country:null,hasBilling:false},
    plan:{id:/** @type {'free'} */('free'),maxLists:1,maxStocks:10,pageSize:25,trialEndsAt:null,expired:false},
    watchlists:[]
  };
}
...
export function readGuestState(storage){
  ...
  try{
    ...
    return {...emptyGuestState(),updatedAt:parsed.updatedAt||new Date().toISOString(),savePromptShown:!!parsed.savePromptShown,watchlists:parsed.watchlists};
  }catch{
...
export function guestHasDraft(state,storage){
  ...
  return data.watchlists.some(list=>list.stocks?.length||list.name);
}
...
export function applyGuestAction(state,input,quote){
  ...
  if(input.action==='createList'){
    ...
    if(next.watchlists.length>=next.plan.maxLists)throw new Error('Watchlist limit reached. Save your list to add more.');
    next.watchlists.push({mode:'advanced',id:crypto.randomUUID(),name,createdAt:new Date().toISOString(),stocks:[],role:'admin',shareToken:null});
    return touch();
  }
  ...
  if(input.action==='addStock'){
    ...
    if(quote.currency&&quote.currency!=='USD')throw new Error('Advanced portfolios currently support USD stocks only.');
