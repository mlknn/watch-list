...
export function applyGuestAction(state,input,quote){
  ...
  if(input.action==='refresh'){
    ...
    next.watchlists=next.watchlists.map(list=>({...list,stocks:list.stocks.map(stock=>{
      ...
      return {...stock,currentPrice:fresh.price,previousClose:Number.isFinite(fresh.previousClose)?fresh.previousClose:null,dayChangePercent:Number.isFinite(fresh.changePercent)?fresh.changePercent:null,quoteTime:fresh.quoteTime,checkedAt:fresh.checkedAt||new Date().toISOString(),quoteError:null,companyName:fresh.companyName||stock.companyName,exchange:fresh.exchange||stock.exchange};
    })}));
    return touch();
  }
  ...
  if(input.action==='addStock'){
    ...
    const wantsPosition=input.quantity!==undefined&&input.quantity!==null&&input.quantity!=='';
    let quantity=null,costPerShare=null,acquiredAt=null;
    if(wantsPosition){
      quantity=Number(input.quantity);
      costPerShare=Number(input.costPerShare??quote.price);
      if(!Number.isFinite(quantity)||quantity<=0||!Number.isFinite(costPerShare)||costPerShare<=0)throw new Error('Enter a positive share quantity and cost per share.');
    ...
    list.stocks.push({
      ...
      previousClose:Number.isFinite(quote.previousClose)?quote.previousClose:null,
      dayChangePercent:Number.isFinite(quote.changePercent)?quote.changePercent:null,
