...
begin
 ...
 if p_action='convertList' then
  if exists(select 1 from public.wl_stocks where watchlist_id=p_list and coalesce(snapshot->>'currency','') not in ('USD','EUR','CAD','TRY')) then
    raise exception 'Portfolios support US (USD), Europe (EUR), Canada (CAD), and Turkey (TRY) stocks.' using errcode='P0001';
  end if;
 ...
 if p_action='addStock' or p_action='initializePosition' then
  ...
  if p_action='initializePosition' or p_quantity is not null then
  ...
  if p_action='addStock' then
    ...
    if v_quote_ccy not in ('USD','EUR','CAD','TRY') then raise exception 'Portfolios support US (USD), Europe (EUR), Canada (CAD), and Turkey (TRY) stocks.' using errcode='P0001';end if;
 ...
 v_id:=public.wl_account_action(p_user,p_action,p_list,p_name,p_symbol,p_quote,p_stock,p_token);
 if p_action='addStock' and v_mode='advanced' and p_quantity is not null then update public.wl_stocks set quantity=p_quantity,cost_per_share=p_cost,acquired_at=p_acquired,notes=p_notes where watchlist_id=p_list and symbol=p_symbol;end if;
 return v_id;
end $$;
commit;
