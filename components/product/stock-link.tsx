...
export function StockLink({symbol,name}:{symbol:string;name:string}){
  ...
  async function show(next:boolean){setOpen(next);if(next){setError('');try{setData(await marketChart(symbol));}catch(e){setError((e as Error).message);}}}const change=data?.quote.changePercent;
