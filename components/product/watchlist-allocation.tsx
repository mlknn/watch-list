...
export function WatchlistAllocation({list,compact=false}:{list:Watchlist;compact?:boolean}){
  ...
  const top=data.slices.slice(0,compact?4:5);
    ...
    <div className="watchlist-alloc-body">
      <div className="watchlist-alloc-chart">
        <PieChart width={size} height={size}>
          <Pie data={data.slices} dataKey="value" nameKey="symbol" innerRadius={compact?36:52} outerRadius={compact?56:78} stroke="none" isAnimationActive={false}>
      ...
      <ol className="watchlist-alloc-list">
        {top.map((slice,index)=><li key={slice.symbol}>
          ...
          <span className="watchlist-alloc-name">{slice.symbol}{!compact&&<small>{slice.name}</small>}</span>
