...
export function SaveAccountPrompt({open,onOpenChange}:{open:boolean;onOpenChange:(open:boolean)=>void;listName?:string}){
  const t=useT();
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="list-dialog save-prompt">
      <DialogTitle>{t("Don’t lose your portfolio")}</DialogTitle>
      <DialogDescription>{t("Create a free account to save this list and come back to it anytime. Everything you entered on this device stays with the account.")}</DialogDescription>
