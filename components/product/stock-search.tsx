...
export function StockSearch({value,onChange,inputRef,onPick,currency}:{value:string;onChange:(value:string)=>void;inputRef:RefObject<HTMLInputElement|null>;onPick?:(symbol:string)=>void;currency?:string}){const t=useT();
 ...
 useEffect(()=>{const controller=new AbortController();setItems(getTickerSuggestions(value,8,currency));setIndex(-1);if(!value.trim()){setLoading(false);return()=>controller.abort();}setLoading(true);const timer=setTimeout(()=>{if(value.trim().length>=2)track('stock_search');void searchTickerSuggestions(value,8,controller.signal,currency).then(result=>{if(!controller.signal.aborted)setItems(result);}).catch(()=>{}).finally(()=>{if(!controller.signal.aborted)setLoading(false);});},180);return()=>{clearTimeout(timer);controller.abort();};},[value,currency]);
