      ...
      {lists.map(list=>{
        ...
        return <li key={list.id}>
          <button type="button" className={list.id===activeId?'is-active':undefined} aria-current={list.id===activeId?'true':undefined} onClick={()=>onSelect(list.id)}>
            ...
            <span className={'watchlist-rail-change'+(change===null?'':change>=0?' up':' down')} title={t("Since created")}>{change===null?'—':`${change>=0?'+':''}${change.toFixed(1)}%`}</span>
          </button>
        </li>;
      })}
    </ul>
