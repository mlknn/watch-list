...
import {Plus,RefreshCw,Eye,Check,Pencil,Trash2,LoaderCircle,AlertCircle,Share2,Link2,Copy,Mail,LockKeyhole,CornerRightDown} from 'lucide-react';
...
export function WatchlistWorkspace({compact=false}:{compact?:boolean}){const t=useT();
  ...
  {state&&active?<>
    ...
    {active.stocks.length?<>{state.guest&&<StockPageHint/>}<CompactStocks stocks={active.stocks.slice(0,5)}/></>:<p className="home-watchlist-empty"><T text="Search for your first stock above. Your watchlist will appear here."/></p>}
...
function WatchlistsOverview({lists,activeId,onSelect}:{lists:Watchlist[];activeId:string;onSelect:(id:string)=>void}){
  ...
  return <table className="watchlist-index">
    ...
    <tbody>{lists.length?lists.map(list=>{
      ...
      return <tr key={list.id} className={list.id===activeId?'is-active':undefined} tabIndex={0} aria-current={list.id===activeId?'true':undefined} onClick={()=>onSelect(list.id)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();onSelect(list.id);}}}><th scope="row">{list.name}</th><td className="watchlist-index-created">{created}</td><td>{list.stocks.length}</td><td className={change===null?'':change>=0?'up':'down'}>{change===null?'—':`${change>=0?'+':''}${change.toFixed(2)}%`}</td></tr>;
...
export function StockPageHint(){
  return <p className="stock-page-hint"><CornerRightDown size={14} aria-hidden="true"/><T text="Click a company to learn more about it"/></p>;
}
...
function CompactStocks({stocks}:{stocks:Stock[]}){
  const t=useT();
  return <div className="home-watchlist-table"><table><caption className="sr-only">{t("Your watchlist")}</caption><thead><tr><th>{t("Company")}</th><th className="shares-column">{t("Shares")}</th><th>{t("Starting price")}</th><th>{t("Latest price")}</th><th>{t("Since added")}</th></tr></thead><tbody>{stocks.map(stock=>{
    const change=stock.addedPrice>0?(stock.currentPrice/stock.addedPrice-1)*100:null;
    return <tr key={stock.id}><th scope="row"><a href={'/stocks/'+encodeURIComponent(stock.symbol)}><CompanyIcon symbol={stock.symbol}/><span>{stock.symbol}<small>{stock.companyName}</small><small className="row-shares">{shareCount(stock.quantity)} {t("Shares")}</small></span></a></th><td className="shares-column">{shareCount(stock.quantity)}</td><td>{price(stock.addedPrice,stock.currency)}</td><td>{price(stock.currentPrice,stock.currency)}</td><td className={change===null?'':change>=0?'up':'down'}>{change===null?'—':`${change>=0?'+':''}${change.toFixed(2)}%`}</td></tr>;
  })}</tbody></table></div>;
}
