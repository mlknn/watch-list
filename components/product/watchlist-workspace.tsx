...
export function WatchlistWorkspace({compact=false}:{compact?:boolean}){const t=useT();
  ...
  async function confirmAdd(e:React.FormEvent){e.preventDefault();if(!active||!addQuote)return;try{const quantity=position.quantity.trim();await act({action:'addStock',listId:active.id,ticker:addQuote.symbol,notes:position.notes,...(quantity?{quantity:Number(quantity),costPerShare:position.cost?Number(position.cost):undefined,acquiredAt:new Date(position.date+'T00:00:00').toISOString()}:{})});setNotice(`${addQuote.symbol} added.`);setAddQuote(null);setTicker('');setPosition(emptyPosition());inputRef.current?.focus({preventScroll:true});}catch(e){setError((e as Error).message);}}
  ...
  {state&&active?<>
    ...
    {active.stocks.length?<CompactStocks stocks={active.stocks.slice(0,5)} hint={!!state.guest}/>:null}
...
</section>:<><header className="topbar"><div className="public-nav-main"><Brand/><ProductNav onNavigate={warnLeave}/></div><div className="account-nav"><ThemeToggle/>{state?.guest?<><a href="/login"><T text="Log in"/></a><button type="button" className="solid-link" onClick={()=>setSaveOpen(true)}><T text="Save watchlist"/></button></>:<><a href="/account"><T text="Account"/></a>{state?.user.analytics&&<a href="/insights">Analytics</a>}<span className="save-state">{busy?<LoaderCircle size={15} className="spin"/>:<Check size={15}/>} {busy?t("Updating…"):state?t("Saved to your account"):t("Connecting…")}</span></>}</div></header>
