...
export function WatchlistWorkspace({compact=false}:{compact?:boolean}){const t=useT();
  ...
  {state&&active?<>
    ...
    {active.stocks.length?<CompactStocks stocks={active.stocks.slice(0,5)}/>:<p className="home-watchlist-empty"><T text="Search for your first stock above. Your watchlist will appear here."/></p>}
...
function CompactStocks({stocks}:{stocks:Stock[]}){
  const t=useT();
  return <div className="home-watchlist-table"><table><caption className="sr-only">{t("Your watchlist")}</caption><thead><tr><th>{t("Company")}</th><th>{t("Starting price")}</th><th>{t("Latest price")}</th><th>{t("Since added")}</th></tr></thead><tbody>{stocks.map(stock=>{
    const change=stock.addedPrice>0?(stock.currentPrice/stock.addedPrice-1)*100:null;
    return <tr key={stock.id}><th scope="row"><a href={'/stocks/'+encodeURIComponent(stock.symbol)}><CompanyIcon symbol={stock.symbol}/><span>{stock.symbol}<small>{stock.companyName}</small></span></a></th><td>{price(stock.addedPrice,stock.currency)}</td><td>{price(stock.currentPrice,stock.currency)}</td><td className={change===null?'':change>=0?'up':'down'}>{change===null?'—':`${change>=0?'+':''}${change.toFixed(2)}%`}</td></tr>;
  })}</tbody></table></div>;
}
