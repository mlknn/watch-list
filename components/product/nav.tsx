...
export function PublicNav() {const t=useT();
 ...
 return <header className="topbar public-nav"><Brand/><nav aria-label="Main navigation"><a href="/#about"><T text="About"/></a><a href="/#how-it-works"><T text="How it works"/></a><a href="/pricing"><T text="Pricing"/></a><a href="/privacy"><T text="Privacy"/></a>{member?<><a className="solid-link" href="/dashboard"><T text="My watchlists"/></a><a href="/account"><T text="Account"/></a><button type="button" className="nav-signout" disabled={busy} onClick={()=>{setBusy(true);setError('');void signOut().catch(()=>{setBusy(false);setError('Could not sign out. Try again.');});}}>{busy?'Signing out…':t("Log out")}</button></>:<><a href="/login"><T text="Log in"/></a><a className="solid-link" href="/signup"><T text="Start watching"/><ArrowUpRight size={16}/></a></>}{error&&<span role="alert">{error}</span>}</nav></header>;
}
