...
export function PublicNav() {
 ...
 return <header className="topbar public-nav"><Brand/><nav aria-label="Main navigation"><a href="/#how-it-works">How it works</a><a href="/pricing">Pricing</a>{member?<><a className="solid-link" href="/dashboard">My watchlists</a><a href="/account">Account</a><button type="button" className="nav-signout" disabled={busy} onClick={()=>{setBusy(true);setError('');void signOut().catch(()=>{setBusy(false);setError('Could not sign out. Try again.');});}}>{busy?'Signing out…':'Log out'}</button></>:<><a href="/login">Log in</a><a className="solid-link" href="/signup">Start watching <ArrowUpRight size={16}/></a></>}{error&&<span role="alert">{error}</span>}</nav></header>;
}
